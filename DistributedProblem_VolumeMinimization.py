...
def MasterProblem(horizon=20, init_vol=[100, 500, 1000]):
    # Create Model 
    m = Model("MP")
    link_up = horizon*10
...
for t in range(0,50):
    ...
    # Stack the things
volume_1 = DATA[0]["P1"]["volume"][0]
for i in range(1, len(DATA)):
    volume_1 = np.append(volume_1, DATA[i]["P1"]["volume"][0])
    
volume_2 = DATA[0]["P2"]["volume"][0]
for i in range(1, len(DATA)):
    volume_2 = np.append(volume_2, DATA[i]["P2"]["volume"][0])
    
volume_3= DATA[0]["P3"]["volume"][0]
for i in range(1, len(DATA)):
    volume_3 = np.append(volume_3, DATA[i]["P3"]["volume"][0])

outflow_1 = DATA[0]["P1"]["outflow"][0]
for i in range(1, len(DATA)):
    outflow_1 = np.append(outflow_1, DATA[i]["P1"]["outflow"][0])
    
outflow_2 = DATA[0]["P2"]["outflow"][0]
for i in range(1, len(DATA)):
    outflow_2 = np.append(outflow_2, DATA[i]["P2"]["outflow"][0])
    
outflow_3= DATA[0]["P3"]["outflow"][0]
for i in range(1, len(DATA)):
    outflow_3 = np.append(outflow_3, DATA[i]["P3"]["outflow"][0])
