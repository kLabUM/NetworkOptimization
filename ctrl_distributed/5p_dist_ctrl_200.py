...
def subproblem(data, volume_lt, volume_ltt, inflows_p1, inflows_p3, inflows_p4):
    ...
    m_p0.addConstr(outflow_p0.sum([str(i) for i in range(0,  190)]) <= volume_lt[0])
    m_p0.addConstr(outflow_p0.sum([str(i) for i in range(190, 200)]) <= volume_ltt[0])
    ...
    # Constarints 
    # Upper limits for flows 
    m_p1.addConstr(outflow_p1.sum([str(i) for i in range(0,160)]) <= volume_lt[1])
    m_p1.addConstr(outflow_p1.sum([str(i) for i in range(160,200)]) <= volume_ltt[1])
    ...
    # Constarints 
    # Upper limits for flows 
    m_p2.addConstr(outflow_p2.sum([str(i) for i in range(0,150)]) <= volume_lt[2])
    m_p2.addConstr(outflow_p2.sum([str(i) for i in range(150,200)]) <= volume_ltt[2])
    ...
    # Upper limits for flows 
    m_p3.addConstr(outflow_p3.sum([str(i) for i in range(0, 170)]) <= volume_lt[3]) # For the channel it is sending water to.
    m_p3.addConstr(outflow_p3.sum([str(i) for i in range(170,200)]) <= volume_ltt[3])
    ...
    # Constarints 
    # Upper limits for flows 
    m_p4.addConstr(outflow_p4.sum([str(i) for i in range(0, 190)]) <= volume_lt[4])
    m_p4.addConstr(outflow_p4.sum([str(i) for i in range(190,200)]) <= volume_ltt[4])
    ...
    inflows_p1[:10] = outflow_p0[190:]

    inflows_p3 = copy.deepcopy(np.asarray(outflow_p2[150:]))
    inflows_p3[:40] = inflows_p3[:40] + np.asarray(outflow_p1[160:])

    inflows_p4 = np.zeros(data["travel_time"][3])
    inflows_p4 = outflow_p3[170:]

    # Residual water that would not enter in the data["horizon"]. 

    # Update the initial volume and inflows for master problem.
    data["qin"][1] = sum(outflow_p0[190:])
    data["qin"][3] = sum(outflow_p1[160:]) + sum(outflow_p2[150:])
    data["qin"][4] = sum(outflow_p3[170:])
