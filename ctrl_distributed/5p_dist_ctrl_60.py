...
def subproblem(data, volume_lt, volume_ltt, inflows_p1, inflows_p3, inflows_p4):
    ...
    # Constarints 
    # Upper limits for flows 
    m_p1.addConstr(outflow_p1.sum([str(i) for i in range(0,20)]) <= volume_lt[1])
    m_p1.addConstr(outflow_p1.sum([str(i) for i in range(20,60)]) <= volume_ltt[1])
    ...
    # Constarints 
    # Upper limits for flows 
    m_p2.addConstr(outflow_p2.sum([str(i) for i in range(0,10)]) <= volume_lt[2])
    m_p2.addConstr(outflow_p2.sum([str(i) for i in range(10,60)]) <= volume_ltt[2])
    ...
    # Upper limits for flows 
    m_p3.addConstr(outflow_p3.sum([str(i) for i in range(0, 30)]) <= volume_lt[3]) # For the channel it is sending water to.
    m_p3.addConstr(outflow_p3.sum([str(i) for i in range(30,60)]) <= volume_ltt[3])
    ...
    inflows_p3 = copy.deepcopy(np.asarray(outflow_p2[10:]))
    inflows_p3[:40] = inflows_p3[:40] + np.asarray(outflow_p1[20:])

    inflows_p4 = np.zeros(data["travel_time"][3])
    inflows_p4 = outflow_p3[30:]
    ...
    data["qin"][3] = sum(outflow_p1[20:]) + sum(outflow_p2[10:])
    data["qin"][4] = sum(outflow_p3[30:])
