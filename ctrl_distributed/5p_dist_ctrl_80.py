...
def subproblem(data, volume_lt, volume_ltt, inflows_p1, inflows_p3, inflows_p4):
    ...
    m_p0.addConstr(outflow_p0.sum([str(i) for i in range(0,  70)]) <= volume_lt[0])
    m_p0.addConstr(outflow_p0.sum([str(i) for i in range(70, 80)]) <= volume_ltt[0])
    ...
    # Constarints 
    # Upper limits for flows 
    m_p1.addConstr(outflow_p1.sum([str(i) for i in range(0,40)]) <= volume_lt[1])
    m_p1.addConstr(outflow_p1.sum([str(i) for i in range(40,80)]) <= volume_ltt[1])
    ...
    # Constarints 
    # Upper limits for flows 
    m_p2.addConstr(outflow_p2.sum([str(i) for i in range(0,30)]) <= volume_lt[2])
    m_p2.addConstr(outflow_p2.sum([str(i) for i in range(30,80)]) <= volume_ltt[2])
    ...
    m_p3.addConstr(outflow_p3.sum([str(i) for i in range(50,80)]) <= volume_ltt[3])
    ...
    # Constarints 
    # Upper limits for flows 
    m_p4.addConstr(outflow_p4.sum([str(i) for i in range(0, 70)]) <= volume_lt[4])
    m_p4.addConstr(outflow_p4.sum([str(i) for i in range(70,80)]) <= volume_ltt[4])
    ...
    inflows_p1[:10] = outflow_p0[70:]

    inflows_p3 = copy.deepcopy(np.asarray(outflow_p2[30:]))
    inflows_p3[:40] = inflows_p3[:40] + np.asarray(outflow_p1[40:])

    inflows_p4 = np.zeros(data["travel_time"][3])
    inflows_p4 = outflow_p3[50:]

    # Residual water that would not enter in the data["horizon"]. 

    # Update the initial volume and inflows for master problem.
    data["qin"][1] = sum(outflow_p0[70:])
    data["qin"][3] = sum(outflow_p1[40:]) + sum(outflow_p2[30:])
