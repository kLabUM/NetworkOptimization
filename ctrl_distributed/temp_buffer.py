...
def subproblem(data, volume_lt, volume_ltt):
    ...
    m_p0.addConstr(outflow_p0.sum([str(i) for i in range(0, data["horizon"] - data["horizon"]%data["travel_time"][0])]) <= volume_lt[0])
    m_p0.addConstr(outflow_p0.sum([str(i) for i in range(data["horizon"] - data["horizon"]%data["travel_time"][0], data["horizon"])]) <= volume_ltt[0])
    ...
    volume_p1  = m_p1.addVars(horizon_t, lb=1, ub=data["max_volume"][1], vtype=GRB.CONTINUOUS)
    outflow_p1 = m_p1.addVars(horizon_t, lb=0, ub=data["channel_flow"][1] , vtype=GRB.CONTINUOUS)

    # Constarints 
    # Upper limits for flows 
    m_p1.addConstr(outflow_p1.sum([str(i) for i in range(0, data["horizon"] - data["horizon"]%data["travel_time"][1])]) <= volume_lt[1])
    m_p1.addConstr(outflow_p1.sum([str(i) for i in range(data["horizon"] - data["horizon"]%data["travel_time"][1], data["horizon"])]) <= volume_ltt[1])
    ...
    # Constarints 
    # Upper limits for flows 
    m_p2.addConstr(outflow_p2.sum([str(i) for i in range(0, data["horizon"] - data["horizon"]%data["travel_time"][2])]) <= volume_lt[2])
    m_p2.addConstr(outflow_p2.sum([str(i) for i in range(data["horizon"] - data["horizon"]%data["travel_time"][2], data["horizon"])]) <= volume_ltt[2])
    ...
    # Upper limits for flows 
    m_p3.addConstr(outflow_p3.sum([str(i) for i in range(0, data["horizon"] - data["horizon"]%data["travel_time"][3])]) <= volume_lt[3]) # For the channel it is sending water to.
    m_p3.addConstr(outflow_p3.sum([str(i) for i in range(data["horizon"] - data["horizon"]%data["travel_time"][3], data["horizon"])]) <= volume_ltt[3])

    # Mass balance 
    m_p3.addConstrs(volume_p3[time] == volume_p3[time-1] - outflow_p3[time] + inflows_p3[time] for time in horizon_t[1:data["travel_time"][1]]) # assuming travel time 1 is > than tt2 
    m_p3.addConstrs(volume_p3[time] == volume_p3[time-1] - outflow_p3[time] + inflows_p3[time] + outflow_p1[time-data["travel_time"][1]] for time in horizon_t[data["travel_time"][1]:data["travel_time"][2]])
    m_p3.addConstrs(volume_p3[time] == volume_p3[time-1] - outflow_p3[time] + outflow_p1[time-data["travel_time"][1]] + outflow_p2[time-data["travel_time"][2]] for time in horizon_t[data["travel_time"][2]:])
    ...
    # Constarints 
    # Upper limits for flows 
    m_p4.addConstr(outflow_p4.sum([str(i) for i in range(0, data["horizon"] - data["horizon"]%data["travel_time"][4])]) <= volume_lt[4])
    m_p4.addConstr(outflow_p4.sum([str(i) for i in range(data["horizon"] - data["horizon"]%data["travel_time"][4], data["horizon"])]) <= volume_ltt[4])

    # Mass balance 
    # inflows_p4 carries water from previous time steps that were skipped in the last data["horizon"]. 

    # This is where the magic happens.
    m_p4.addConstrs((volume_p4[time] == volume_p4[time-1] - outflow_p4[time] + inflows_p4[time] for time in horizon_t[1:data["travel_time"][3]]))
    m_p4.addConstrs((volume_p4[time] == volume_p4[time-1] - outflow_p4[time] + outflow_p3[time-data["travel_time"][3]] for time in horizon_t[data["travel_time"][3]:]))
    ...
    inflows_p1 = np.zeros(data["travel_time"][0])
    inflows_p1[:len(outflow_p1[data["horizon"] - data["horizon"]%data["travel_time"][0]: data["horizon"]])] = outflow_p1[data["horizon"] - data["horizon"]%data["travel_time"][0]: data["horizon"]]

    inflows_p3 = np.zeros(data["travel_time"][2])
    inflows_p3[:len(outflow_p2[data["horizon"] - data["horizon"]%data["travel_time"][2]: data["horizon"]])] = outflow_p2[data["horizon"] - data["horizon"]%data["travel_time"][2]: data["horizon"]]
    inflows_p3[:len(outflow_p1[data["horizon"] - data["horizon"]%data["travel_time"][1]: data["horizon"]])] = inflows_p3[:len(outflow_p1[data["horizon"] - data["horizon"]%data["travel_time"][1]: data["horizon"]])] + outflow_p1[data["horizon"] - data["horizon"]%data["travel_time"][1]: data["horizon"]]

    inflows_p4 = np.zeros(data["travel_time"][3])
    inflows_p4[:len(outflow_p3[data["horizon"] - data["horizon"]%data["travel_time"][3]: data["horizon"]])] = outflow_p3[data["horizon"] - data["horizon"]%data["travel_time"][3]: data["horizon"]]

    # Residual water that would not enter in the data["horizon"]. 

    # Update the initial volume and inflows for master problem.
    data["qin"][1] = sum(outflow_p0[data["horizon"] - data["horizon"]%data["travel_time"][0]:])

    data["qin"][3] = sum(outflow_p1[data["horizon"] - data["horizon"]%data["travel_time"][1]:])
    data["qin"][3] += sum(outflow_p2[data["horizon"] - data["horizon"]%data["travel_time"][2]:])
    data["qin"][4] = sum(outflow_p3[data["horizon"] - data["horizon"]%data["travel_time"][3]:])

    data["vt0"][0] = volume_p0[data["horizon"]-1]
    data["vt0"][1] = volume_p1[data["horizon"]-1]
    data["vt0"][2] = volume_p2[data["horizon"]-1]
    data["vt0"][3] = volume_p3[data["horizon"]-1]
    data["vt0"][4] = volume_p4[data["horizon"]-1]
    ...
    return data, outflow_ponds, volume_ponds


# Initial Conditions 
INIT_VOLUME = [1000, 800, 1000, 1000, 1]

# Physical Constraints
data = {}
data["nodes"] = [0, 1, 2, 3, 4]
data["arcs"] = [0, 1, 2, 3, 4]
data["max_volume"]   = [1000, 1000, 1000, 1000, 1000]
...
for i in range(0,10):
    ...
    data, outflow_pond, volume_ponds = subproblem(data, outflows_t, outflows_tt)
    for i in data["nodes"]:
