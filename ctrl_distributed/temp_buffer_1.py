...
def subproblem(data, volume_lt, volume_ltt, inflows_p1, inflows_p3, inflows_p4):
    ...
    m_p0.addConstr(outflow_p0.sum([str(i) for i in range(0,  50)]) <= volume_lt[0])
    m_p0.addConstr(outflow_p0.sum([str(i) for i in range(50, 60)]) <= volume_ltt[0])
    ...
    # Constarints 
    # Upper limits for flows 
    m_p1.addConstr(outflow_p1.sum([str(i) for i in range(0,50)]) <= volume_lt[1])
    m_p1.addConstr(outflow_p1.sum([str(i) for i in range(50,60)]) <= volume_ltt[1])
    ...
    m_p2.addConstr(outflow_p2.sum([str(i) for i in range(50,60)]) <= volume_ltt[2])
    ...
    # Upper limits for flows 
    m_p3.addConstr(outflow_p3.sum([str(i) for i in range(0, 50)]) <= volume_lt[3]) # For the channel it is sending water to.
    m_p3.addConstr(outflow_p3.sum([str(i) for i in range(50,60)]) <= volume_ltt[3])

    # Mass balance 
    m_p3.addConstrs(volume_p3[time] == volume_p3[time-1] - outflow_p3[time] + inflows_p3[time] for time in horizon_t[1:10]) # assuming travel time 1 is > than tt2 
    m_p3.addConstrs(volume_p3[time] == volume_p3[time-1] - outflow_p3[time] + inflows_p3[time] + outflow_p1[time-data["travel_time"][1]] for time in horizon_t[10:10])
    m_p3.addConstrs(volume_p3[time] == volume_p3[time-1] - outflow_p3[time] + outflow_p1[time-data["travel_time"][1]] + outflow_p2[time-data["travel_time"][2]] for time in horizon_t[10:])
    ...
    # Constarints 
    # Upper limits for flows 
    m_p4.addConstr(outflow_p4.sum([str(i) for i in range(0, 50)]) <= volume_lt[4])
    m_p4.addConstr(outflow_p4.sum([str(i) for i in range(50,60)]) <= volume_ltt[4])

    # Mass balance 
    # inflows_p4 carries water from previous time steps that were skipped in the last data["horizon"]. 

    # This is where the magic happens.
    m_p4.addConstrs((volume_p4[time] == volume_p4[time-1] - outflow_p4[time] + inflows_p4[time] for time in horizon_t[1:10]))
    ...
    inflows_p1[:10] = outflow_p0[50:]

    inflows_p3 = np.zeros(data["travel_time"][2])
    inflows_p3[:10] = np.asarray(outflow_p2[50:]) + np.asarray(outflow_p1[50:])

    inflows_p4 = np.zeros(data["travel_time"][3])
    inflows_p4[:10] = outflow_p3[50:]

    # Residual water that would not enter in the data["horizon"]. 

    # Update the initial volume and inflows for master problem.
    data["qin"][1] = sum(outflow_p0[50:])
    data["qin"][3] = sum(outflow_p1[50:]) + sum(outflow_p2[50:])
    data["qin"][4] = sum(outflow_p3[50:])
...
data["arcs"] = [0, 1, 2, 3, 4]
data["max_volume"]   = [2000, 2000, 2000, 2000, 2000]
data["channel_flow"] =  [5, 10, 5, 10, 10]
data["travel_time"]  = [10, 10, 10, 10, 10]
data["weights"]  = [1.0, 1.0, 1.0, 10.0, 1.0]
...
# Physical Constraints
data1 = {}
data1["nodes"] = [0, 1, 2, 3, 4]
data1["arcs"] = [0, 1, 2, 3, 4]
data1["max_volume"]   = [1000, 1000, 1000, 1000, 1000]
data1["channel_flow"] =  [5, 10, 5, 10, 10]
data1["travel_time"]  = [10, 40, 50, 30, 10]
data1["weights"]  = [1.0, 1.0, 1.0, 0.5, 1.0]
data1["vt0"] = INIT_VOLUME1
data1["qin"] = [0, 0, 0, 0, 0]
data1["horizon"] = 250
...
for i in range(0,4):
    volumes1, outflows_t1, outflows_tt1 = master(data1)
    data1, outflow_pond1, volume_ponds1, inflows_p1, inflows_p3, inflows_p4  = subproblem(data1, outflows_t1, outflows_tt1, inflows_p1, inflows_p3, inflows_p4)
    for i in data["nodes"]:
        for v,o in zip(volume_ponds1[i], outflow_pond1[i]):
            volume1[i].append(v)
            outflow1[i].append(o)
"""
